...
# ================= 💾 DATABASE (MONGODB) =================
# maxPoolSize=100 kar diya hai taaki DB connection me kabhi lag na aaye
...
def get_unjoined(uid):
    ...
    for ch in list(channels_col.find()):
        try:
            ...
            # Agar user ne join nahi kiya hai, aur uski join request pending bhi nahi hai, toh wo unjoined hai
...
# 🔥 SMART FEATURE: Try Again Button Logic
...
def verify(call):
    ...
    if not unjoined:
        ...
        # Agar saare join nahi kiye, toh sirf bache hue channels dikhao aur "Try Again" lagao
        markup = InlineKeyboardMarkup()
        for ch in unjoined:
            markup.add(InlineKeyboardButton("Join Channel", url=ch['link']))
        markup.add(InlineKeyboardButton("🔄 Try Again", callback_data="verify")) # Naya Try Again Button
        
        try:
            bot.edit_message_reply_markup(call.message.chat.id, call.message.message_id, reply_markup=markup)
        except: pass # Agar same message ho toh error se bachne ke liye
